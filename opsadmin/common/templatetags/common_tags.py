...
@register.simple_tag(name='random_code')
def random_code(value):
    return random.randint(10, 1000) - value

@register.simple_tag(name='check_list_empty')
def check_list_empty(value):
    if len(value) == 0:
        return True
    else:
        return False

@register.simple_tag(name='get_asset_obj')
def get_asset_obj(value):
    return Asset.objects.filter(id=value)[0]
...
def paginate(context, object_list, page_count):
    ...
    context['list'] = object_list # 把获取到的分页的数据封装到上下文中
